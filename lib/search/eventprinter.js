...
function printStart(data, prevTime, length) {
  ...
  line += '>';
  _print(data.timeL, prevTime, line, length, 2);
}
...
function printMsg(data, prevTime, lineLength) {
  ...
  const msg = data.messageData.msg.text.ellipse(length);
