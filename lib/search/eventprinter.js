...
function buildStart(data, prevTime, length) {
  ...
  for (let i = 0; i < prevPadLength - 1; i += 1) {
    line = `=${line}`;
  }
  for (let i = 0; i < nextPadLength - 3; i += 1) {
    line += '=';
  }
  line += '>';
...
function printMsg(data, prevTime, lineLength) {
  ...
  if (data.sentBy === 'Consumer') {
    ...
    sentBy = (`[${`${participants[data.participantId] || data.participantId}`.padEnd(sentByLength, ' ')}]`).blue;
    if (participants[data.participantId]
...
function printTransfer(data, prevTime, lineLength) {
  const by = `${participants[data.by] || data.by}`;
...
function printDaemon(data, prevTime, length, dVerb) {
  ...
  } else if (data.sent_to_b2q === 'Y') {
    dMsg = '  [Daemon Check]'.magenta + ` sent_to_b2q  ${data.b2q_err_code}`.gray;
  } else if (dVerb) {
    dMsg = '  [Daemon Check]'.magenta + ' no action'.gray;
  } else {
    _updateDuration(data.task_date, prevTime);
    return '';
  }
...
function _updateDuration(timeL, prevTime) {
  duration += (timeL - prevTime);
}

function _buildEvent(timeL, prevTime, msg, lineLength, prevBl, nextBl) {
  let ret = '';
  _updateDuration(timeL, prevTime);
