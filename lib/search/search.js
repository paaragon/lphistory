...
async function printLpHistory(conversationId, timeShift, lineLength, env) {
  try {
...
function handleError(e, tsShift) {
  if (isWrongTimestamp(e)) {
    const { tsStart, tsEnd } = extractValidTimestamps(e);
    const sendTs = extractCurrentTimestamp(e);
    if (!tsStart || !tsEnd || !sendTs) {
      console.log('Error inesperado');
    } else {
      console.log(`    Worng OAuth timestamp`.red);
      suggestTimestampShift(parseInt(tsStart), parseInt(tsEnd), parseInt(sendTs), parseInt(tsShift));
    }
...
function suggestTimestampShift(tsStart, tsEnd, sendTs, tsShift) {
  ...
  console.log(`Invalid OAuth timestamp provided`);
  console.log(`Suggested -t argument value: ${sugTs}`);
}
